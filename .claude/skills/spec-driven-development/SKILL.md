...
Execute tasks one at a time following `skills/incremental-implementation/SKILL.md` (`incremental-implementation`) and `skills/test-driven-development/SKILL.md` (`test-driven-development`). At each step, load only the spec sections and source files that task needs rather than flooding the agent with the entire spec.
